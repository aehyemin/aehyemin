...
PROJECTS = [
    ...
    },
    {
        ...
        "labels": {"chal": "01 DATASET - 쟁점 기준 채점표",
                   ...
                   "fix": "03 EVAL - 점수와 근거 신뢰도를 분리 측정"},
        "challenge": [
            "국내 법률상담 서비스의 실제 이용자 질문과, 법률 AI 가 작성하고 "
            "변호사가 검토·수정한 답변을 골드셋으로 사용",
            "골드셋 QA 를 쟁점 단위로 묶어 <b>IRAC 항목 단위 채점표</b> 생성 "
            "(채점 항목 1,271개)",
        ],
        "cause": [
            "<u>모범답변도 인용 목록도 주지 않음</u>"
            "(무엇을 찾을지 판단하는 것부터 과제)",
            "도구 5종, 최대 30턴(법령·판례 검색, 문서 열람, 정보 추출, 답변 제출)",
            "도구 조건과 무도구 조건을 같은 하네스에서 실행",
        ],
        "fix": [
            "채점표 항목마다 <b>0 / 1</b>, 문항별 통과율의 평균이 점수"
            "(판정 모델 Opus-5)",
            "답변이 인용한 조문·판례 <b>9,878건</b>을 공식 API 로 조회해 "
            "실존과 내용 일치를 대조",
            "인용 정확도를 점수와 독립된 축으로 보고",
        ],
        "res_lead": [
            "도구 조건에서 4모델 모두 답변 품질 상승"
            "(<b>0.744 → 0.794</b>, 양쪽 조건에서 답을 낸 579문항 기준)",
            "축별로는 법리(R)에서 가장 크게 오름(<b>+0.087</b>) — "
            "결론(C, +0.027)보다 큼",
            "판례 인용 유효율 <b>0.71 → 0.86</b>, "
            "핵심 근거가 틀린 답변 비율 <b>0.33 → 0.17</b>",
            "인용 오류 중 <u>존재하지 않는 인용은 5.6%</u>이고 나머지는 실재하는 "
            "조문·판례에 없는 내용을 붙인 것 → 사건번호만 대조하는 검증으로는 "
            "탐지되지 않는 유형",
            "도구를 다루는 능력 자체가 성능을 가름 → 검색 0건 비율이 모델별 "
            "24~50%로 갈리고, 낮은 모델일수록 점수가 높음. 상위 모델은 결과가 "
            "0건이면 검색어를 즉시 줄이지만, 하위 모델은 같은 질의를 반복하다 "
            "턴 한도를 소진",
        ],
        "limits": [
            "gemma 는 도구 조건에서 11문항이 턴 한도 초과로 미제출(완주율 0.93) — "
            "미답변을 0점으로 세는 전체 집계에서는 −0.015. 답변 품질 저하가 아니라 "
            "에이전트 루프 적합성의 문제이므로 완주율을 점수와 함께 읽어야 함",
            "채점 기준과 인용 판정을 모두 LLM 이 수행(사람과의 일치도 검증 진행)",
            "모범답안이 상한이므로 그보다 나은 답변은 변별하지 못함",
        ],
